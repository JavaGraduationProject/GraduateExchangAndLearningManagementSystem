import Vue from 'vue';
//配置路由
import VueRouter from 'vue-router'
Vue.use(VueRouter);
//1.创建组件
import Index from '@/views/index'
import Home from '@/views/home'
import Login from '@/views/login'
import NotFound from '@/views/404'
import UpdatePassword from '@/views/update-password'
import pay from '@/views/pay'
import register from '@/views/register'
import center from '@/views/center'
    import discusswenxianziliao from '@/views/modules/discusswenxianziliao/list'
    import xuesheng from '@/views/modules/xuesheng/list'
    import discusskaoyanziliao from '@/views/modules/discusskaoyanziliao/list'
    import kaoyanziliao from '@/views/modules/kaoyanziliao/list'
    import discusskaogongziliao from '@/views/modules/discusskaogongziliao/list'
    import jiaoshi from '@/views/modules/jiaoshi/list'
    import storeup from '@/views/modules/storeup/list'
    import discusszhaopinxinxi from '@/views/modules/discusszhaopinxinxi/list'
    import forum from '@/views/modules/forum/list'
    import zhaopinxinxi from '@/views/modules/zhaopinxinxi/list'
    import wenxianziliao from '@/views/modules/wenxianziliao/list'
    import jianlitoudi from '@/views/modules/jianlitoudi/list'
    import kaogongziliao from '@/views/modules/kaogongziliao/list'
    import config from '@/views/modules/config/list'


//2.配置路由   注意：名字
const routes = [{
    path: '/index',
    name: '首页',
    component: Index,
    children: [{
      // 这里不设置值，是把main作为默认页面
      path: '/',
      name: '首页',
      component: Home,
      meta: {icon:'', title:'center'}
    }, {
      path: '/updatePassword',
      name: '修改密码',
      component: UpdatePassword,
      meta: {icon:'', title:'updatePassword'}
    }, {
      path: '/pay',
      name: '支付',
      component: pay,
      meta: {icon:'', title:'pay'}
    }, {
      path: '/center',
      name: '个人信息',
      component: center,
      meta: {icon:'', title:'center'}
    }
      ,{
	path: '/discusswenxianziliao',
        name: '文献资料评论',
        component: discusswenxianziliao
      }
      ,{
	path: '/xuesheng',
        name: '学生',
        component: xuesheng
      }
      ,{
	path: '/discusskaoyanziliao',
        name: '考研资料评论',
        component: discusskaoyanziliao
      }
      ,{
	path: '/kaoyanziliao',
        name: '考研资料',
        component: kaoyanziliao
      }
      ,{
	path: '/discusskaogongziliao',
        name: '考公资料评论',
        component: discusskaogongziliao
      }
      ,{
	path: '/jiaoshi',
        name: '教师',
        component: jiaoshi
      }
      ,{
	path: '/storeup',
        name: '我的收藏管理',
        component: storeup
      }
      ,{
	path: '/discusszhaopinxinxi',
        name: '招聘信息评论',
        component: discusszhaopinxinxi
      }
      ,{
	path: '/forum',
        name: '学习交流',
        component: forum
      }
      ,{
	path: '/zhaopinxinxi',
        name: '招聘信息',
        component: zhaopinxinxi
      }
      ,{
	path: '/wenxianziliao',
        name: '文献资料',
        component: wenxianziliao
      }
      ,{
	path: '/jianlitoudi',
        name: '简历投递',
        component: jianlitoudi
      }
      ,{
	path: '/kaogongziliao',
        name: '考公资料',
        component: kaogongziliao
      }
      ,{
	path: '/config',
        name: '轮播图管理',
        component: config
      }
    ]
  },
  {
    path: '/login',
    name: 'login',
    component: Login,
    meta: {icon:'', title:'login'}
  },
  {
    path: '/register',
    name: 'register',
    component: register,
    meta: {icon:'', title:'register'}
  },
  {
    path: '/',
    name: '首页',
    redirect: '/index'
  }, /*默认跳转路由*/
  {
    path: '*',
    component: NotFound
  }
]
//3.实例化VueRouter  注意：名字
const router = new VueRouter({
  mode: 'hash',
  /*hash模式改为history*/
  routes // （缩写）相当于 routes: routes
})

export default router;
